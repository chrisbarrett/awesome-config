...
local function open_in_emacs(err, notification)
  local path = "/tmp/awesome-error.stacktrace"
  local file = io.open(path, 'w')
  file:write(tostring(err))
  file:close()

  awful.spawn({ "emacsclient", "--alternate-editor=emacs",
                "--eval", '(with-current-buffer (find-file "' .. path  .. '") (compilation-minor-mode))' })
end
